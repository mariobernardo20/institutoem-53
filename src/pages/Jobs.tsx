...
const Jobs = () => {
  ...
  useEffect(() => {
    let filtered = jobs;
    if (searchTerm) {
      filtered = filtered.filter(job => job.title.toLowerCase().includes(searchTerm.toLowerCase()) || job.company.toLowerCase().includes(searchTerm.toLowerCase()));
    }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="relative">
            ...
            <Input placeholder="Pesquisar vagas..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
          </div>
        ...
        <div className="grid gap-6">
          {filteredJobs.map(job => <Card key={job.id} className="hover:shadow-md transition-shadow">
              <CardHeader>
              ...
              <CardContent>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {job.requirements.map((req, index) => <Badge key={index} variant="outline" className="text-xs">
                        {req}
                      </Badge>)}
                  </div>
                </div>
