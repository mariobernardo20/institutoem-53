...
const Scholarships = () => {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(scholarship => scholarship.title.toLowerCase().includes(searchTerm.toLowerCase()) || scholarship.provider.toLowerCase().includes(searchTerm.toLowerCase()) || scholarship.category.toLowerCase().includes(searchTerm.toLowerCase()));
    }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="relative">
            ...
            <Input placeholder="Pesquisar bolsas..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
          </div>
        ...
        <div className="grid gap-6">
          {filteredScholarships.map(scholarship => <Card key={scholarship.id} className="hover:shadow-md transition-shadow">
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {isDeadlineApproaching(scholarship.deadline) && <Badge variant="destructive" className="text-xs">
                          Prazo próximo!
                        </Badge>}
                    </div>
              ...
              <CardContent>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {scholarship.eligibility.map((criteria, index) => <Badge key={index} variant="outline" className="text-xs">
                        {criteria}
                      </Badge>)}
                  </div>
                </div>

                <Button className="w-full md:w-auto" onClick={() => window.open(scholarship.link, '_blank')}>
                  Mais informações
                </Button>
              </CardContent>
            </Card>)}
        ...
        {filteredScholarships.length === 0 && <div className="text-center py-12">
