...
const Candidates = () => {
  ...
  const filterCandidates = () => {
    ...
    // Search filter
    if (searchTerm) {
      filtered = filtered.filter(candidate => candidate.name.toLowerCase().includes(searchTerm.toLowerCase()) || candidate.area.toLowerCase().includes(searchTerm.toLowerCase()) || candidate.skills.some(skill => skill.toLowerCase().includes(searchTerm.toLowerCase())));
    }
    ...
    if (areaFilter && areaFilter !== "all") {
      filtered = filtered.filter(candidate => candidate.area.toLowerCase().includes(areaFilter.toLowerCase()));
    }
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="relative">
                ...
                <Input placeholder="Pesquisar candidatos..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCandidates.map(candidate => <Card key={candidate.id} className="hover:shadow-lg transition-shadow">
              <CardContent className="p-6">
                ...
                <div className="relative mb-6">
                  <div className="flex items-center justify-center mb-4">
                    <div className="w-20 h-20 bg-primary/10 rounded-full flex items-center justify-center border-4 border-white shadow-md">
                      {candidate.image_url ? <img src={candidate.image_url} alt={candidate.name} className="w-20 h-20 rounded-full object-cover" /> : <span className="text-2xl font-bold text-primary">
                ...
                <div className="mb-4">
                  <div className="flex flex-wrap gap-2 justify-center">
                    {candidate.skills.slice(0, 3).map((skill, index) => <Badge key={index} variant="outline" className="text-xs">
                        {skill}
                      </Badge>)}
                ...
                <div className="space-y-2 mb-4">
                  {candidate.email && <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                      <Mail className="h-4 w-4" />
                      <span className="truncate">{candidate.email}</span>
                    </div>}
                  {candidate.phone && <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                ...
                {/* Action */}
                <Button className="w-full" size="sm" onClick={() => navigate(`/candidato/${candidate.id}`)}>
                  Ver Perfil Completo
                </Button>
              </CardContent>
            </Card>)}
